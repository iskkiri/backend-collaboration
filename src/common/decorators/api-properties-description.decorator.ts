...
interface ApiPropertiesDescriptionOptions {
  dto: new () => unknown;
  description?: string;
}

export function ApiPropertiesDescription(options: ApiPropertiesDescriptionOptions) {
  const { dto, description } = options;
  const schemas = Reflect.getMetadata(DECORATORS.API_MODEL_PROPERTIES_ARRAY, dto.prototype) || [];
